...
import { Section, Card as CardType } from "../../../types";
...
const Card = (props: Props) => {
  ...
  const cardData = rawData
    .find((section: Section) =>
      section.cards.some((card: CardType) => card.id === card_id)
    )
    ?.cards.find((card: CardType) => card.id === card_id);
  ...
  return (
    <div className=" w-full min-h-screen flex justify-center bg-white">
      <div className=" flex flex-col w-2/3">
        ...
        <div className=" flex flex-row items-center justify-center gap-2">
          ...
          <div className=" flex flex-col items-start justify-center">
            ...
            <Link
              href={cardData?.link || ""}
              target="_blank"
              className=" text-sm bg-blue-500 mt-4 p-2 rounded-md text-white"
            >
              链接直达
            </Link>
          </div>
        ...
        {/* 描述 */}
        <div className="text-sm text-gray-500">{cardData?.description}</div>
        {/* 广告位 */}
